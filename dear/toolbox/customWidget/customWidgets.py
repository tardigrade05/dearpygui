...
class LabelInput:
    def __init__(
        ...
        with dpg.group(horizontal=True, indent=group_indent):
            ...
            dpg.add_input_text(tag=self.input_tag,indent=input_indent, width=input_width, decimal=input_decimal,readonly = input_readonly,default_value=default_input_val)
...
class LabelInputLabel:
    def __init__(
        ...
        with dpg.group(horizontal=True, indent=group_indent):
            dpg.add_text(default_value = label_name, tag= self.label_tag)
            dpg.add_input_text(tag=self.input_tag,indent=input_indent, width=input_width, decimal=input_decimal,readonly = input_readonly) #,callback = self.press
            dpg.add_text(default_value = '0', tag= self.label_product_tag)
